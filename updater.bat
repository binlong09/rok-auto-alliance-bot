...
REM   %3 = exe name to relaunch (e.g. "RoK Automation.exe")
REM
REM Spawned by src/updater.py (from a copy in %TEMP%, never from the
REM install dir - cmd reads bat files incrementally, so a bat inside the
REM install dir could be overwritten by its own robocopy mid-run). The
REM main exe exits right after spawning this so its files can be replaced.
...
cd /d "%INSTALL_DIR%"
REM Use PowerShell Start-Process, not cmd's `start`: when the bat runs
